...
const SignalGenerator: React.FC = () => {
  ...
  class ProfessionalSMCEngine {
    ...
    private getSymbolVolatility(symbol: string) {
      const volatilityMap: {[key: string]: number} = {
        ...
        'XAU/USD': 0.002, 'XAG/USD': 0.003, 'BTCUSDT': 0.02,
        'ETHUSDT': 0.025, 'USOIL': 0.015
      };
    ...
    createTradingSignal(symbol: string, analysis: any, currentPrice: number, timeframe: string, market: 'crypto' | 'forex') {
      const confidence = this.calculateConfidence(analysis.confirmations);
      const priceHistory = this.priceHistory.get(symbol);
      ...
      return {
        id: `signal-${symbol}-${timeframe}-${Date.now()}`,
        symbol,
  ...
  const smcEngine = useRef(new ProfessionalSMCEngine());
  ...
  const fetchForexPrice = async (symbol: string) => {
    // Simulate real forex price
  ...
  const performAdvancedSMCAnalysis = async (symbol: string, priceData: any, timeframe: string) => {
    try {
      ...
      if (analysisResult && smcEngine.current.validateSignalQuality(analysisResult)) {
        ...
        const signalForUser = {
          id: parseInt(signal.id.split('-')[3]),
          text: `${signal.symbol}\n${signal.signalType} NOW\nEntry ${signal.entryPrice}\nStop Loss ${signal.stopLoss}\nTake Profit ${signal.takeProfit}\nConfidence ${signal.confidence}%\n\n${signal.analysis}`,
          timestamp: signal.timestamp.toISOString(),
          from: 'Auto Signal Generator',
          chat_id: 1,
          message_id: parseInt(signal.id.split('-')[3]),
          update_id: parseInt(signal.id.split('-')[3])
        };
  ...
  const startAnalysis = async () => {
    ...
    const symbols = selectedSymbol === 'ALL' ? markets[activeTab].symbols : [selectedSymbol];
    ...
    const runAnalysis = async () => {
      ...
      for (const symbol of symbols) {
        for (const timeframe of timeframes) {
          try {
            const priceData = activeTab === 'crypto' 
  ...
  const stopAnalysis = () => {
    ...
    addLog('⏹️ Analysis stopped.', 'warning');
  };
  ...
  const refreshSystem = () => {
    ...
    addLog('🔄 System refreshed.', 'success');
  };

  const copyTradeDetails = (signal: Signal) => {
  ...
  useEffect(() => {
    addLog('🚀 SMC Signal Generator initialized', 'success');
    addLog('📊 Ready to generate professional trading signals', 'info');
      ...
      <div className="bg-gray-900/50 backdrop-blur-sm rounded-xl border border-cyan-500/30 p-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white tracking-wider">SMC Signal Generator</h2>
              <p className="text-gray-400">Professional Smart Money Concepts Analysis</p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              >
                ...
                <option value="ALL" className="bg-red-600 text-white font-bold">📊 ALL SYMBOLS</option>
                {markets[activeTab].symbols.map(symbol => (
            ...
            <div>
              ...
              >
                ...
                <option value="ALL" className="bg-red-600 text-white font-bold">⏰ ALL TIMEFRAMES</option>
                {timeframes.map(tf => (
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto futuristic-scrollbar">
            {Object.keys(marketData).length === 0 ? (
              <div className="text-center py-8 text-gray-400">
                ...
                <p className="text-xs mt-2">Only verified prices will appear</p>
              </div>
            ) : (
